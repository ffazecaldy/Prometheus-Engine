...
**Commit locale: SEMPRE per singolo task** (economico, atomico, rollback granulare — non condizionato).

**Push remoto: dipende dalla preferenza esplicita dell'utente**, verificata a inizio Fase 0.5 (Clarification Interview) o dedotta dal prompt corrente:

```
RILEVAMENTO PREFERENZA PUSH (controlla il prompt utente PRIMA del primo commit):

├─ Utente dice esplicitamente "pusha solo alla fine" / "un commit solo" / "no push intermedi":
│   └─ push_mode = "batch_end"
│   └─ Commit locali per ogni task, PUSH UNICO a fine batch/sessione
│
├─ Utente dice esplicitamente "pusha quando vuoi" / "pusha come preferisci" / "gestisci tu i push":
│   └─ push_mode = "per_task"
│   └─ Segui la regola di default sotto (push dopo ogni task che supera la soglia)
│
├─ Utente non specifica nulla:
│   └─ push_mode = "per_task" (default)
│   └─ Push dopo ogni task che supera la soglia, come descritto sotto
│
└─ Se push_mode non è ancora determinato quando arriva il primo task pronto per il commit,
   NON bloccare il loop: usa il default (per_task) e prosegui.
```

**Comportamento per `push_mode = "per_task"` (default):**

```
GIT CHECKPOINT RULES (per_task):
...
**Comportamento per `push_mode = "batch_end"`:**

```
GIT CHECKPOINT RULES (batch_end):
1. Commit locale subito dopo che un task supera la soglia (sempre, indipendente dal push)
2. Push UNICO dopo che:
   ├─ Tutti i task del batch sono committati localmente
   └─ L'Assembly Task ha risolto eventuali gap cross-file
3. Se la sessione è multi-batch (Tier 4): un push per batch preferibile a un push per sessione
   └─ Non accumulare troppi commit locali non pushati (rischio perdita in caso di crash)
```
...
6. 🌉 ASSEMBLY TASK: dopo che TUTTI i task del batch sono verificati, un assembly task dedicato:
   ...
   ├─ 🔍 DRY CHECK (Tier 3-4): scansiona i moduli prodotti in parallelo per rilevare helper o logiche duplicate
...
> **⚠️ CI/CD awareness:** se il progetto ha CI/CD (`.github/workflows/`, `.gitlab-ci.yml`), `push_mode = "per_task"` con Tier 3-4 (10-40+ task) può scatenare altrettante esecuzioni di pipeline. Se l'utente non specifica nulla ma la CI è rilevabile, segnalalo esplicitamente e proponi `batch_end` come alternativa più economica.
